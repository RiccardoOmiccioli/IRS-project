...
function is_calibration_needed()
    ...
    back_distance = get_back_distance()
...
function calibrate_position()
    local front_distance = get_front_distance()
    local back_distance = get_back_distance()
    ...
    if front_distance > 0 then
        if front_distance < SENSOR_DISTANCE_FROM_WALL - POSITION_ERROR_THRESHOLD then
            ...
            move(BASIC_MOVE.STRAIGHT, MOVE_DIRECTION.BACKWARDS, move_distance, true)
        elseif front_distance > SENSOR_DISTANCE_FROM_WALL + POSITION_ERROR_THRESHOLD and front_distance < MAX_SHORT_RANGE_DISTANCE then
            ...
            move(BASIC_MOVE.STRAIGHT, MOVE_DIRECTION.FORWARD, move_distance, true)
        end
